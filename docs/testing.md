...
- 174 rules/API tests passed, including four complete matches, validated attacker gambles, suit/rank public-card freshness leads, supported declaration tie-breaking, tactical follows, failed-gamble feedback, snapshot restart recovery, failed writes, corrupt files, and expiry.
- Twelve Playwright journeys passed across desktop Chrome and mobile Chrome emulation. Bot journeys cover paced turns, pause/skip controls, a complete round, history, resume after refresh, and starting round two.
