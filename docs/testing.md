...
The foundation does not claim coverage of timed dealing/declarations, complete live follow enforcement outside the guided positions (D6, D12), full gamble decomposition/comparison (D9–D10), actual kitty settlement, Jack reset (D8), joker declarations (D11), or complete match lifecycle. Add executable fixtures for each when those modules are implemented. The three-joker/four-joker and unresolved higher-multiplicity examples in the decision log remain requirements, not passing tests.
